...
LineTransformer.prototype.capitalizeDot = function(content){
  return this.capitalize(content).replace(/[\.!\?]\s*(\D)/g, function(s, res){
    return s.substring(0, s.length-1) + s[s.length-1].toUpperCase();
  });
};

LineTransformer.prototype.trim = function(content){
  return content.trim();
};
